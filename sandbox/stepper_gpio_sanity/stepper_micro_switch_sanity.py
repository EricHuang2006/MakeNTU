...
def monitor_switch(interval_s):
    print("Monitoring bottom switch. Press Ctrl+C to stop.")
    while True:
        read_switch("switch")
...
def run_until_switch_test(max_cm, direction, progress_interval_steps):
    ...
    print(
        f"direction={direction}, max_cm={max_cm}, max_steps={max_steps}, "
        f"progress_interval_steps={progress_interval_steps}"
    )
    wait_for_enter(
        "Release the switch. The motor will move until the switch is pressed, "
        "or until the max distance is reached."
    )
    ...
    for step_index in range(max_steps):
        ...
        if progress_interval_steps > 0 and moved % progress_interval_steps == 0:
            print(f"moved_steps={moved}, switch_pressed={axis.bottom_switch_pressed()}")
    ...
    if not pressed:
        raise RuntimeError(
            "Switch was not pressed before the bounded movement limit. "
...
def main():
    ...
    try:
        if args.monitor_switch:
            monitor_switch(args.monitor_interval_s)
            return

        if args.until_switch:
            run_until_switch_test(
                args.until_switch_max_cm,
                args.home_direction,
