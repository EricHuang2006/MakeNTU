...
def evaluate_photo_quality(indices, boxes, all_keypoints, img_size):
    ...
    Inputs:
        indices: final person indices after NMS
        boxes: detected boxes, each [x, y, w, h]
        all_keypoints: 17 COCO keypoints for each detected person
        img_size: image size, currently 320

    Returns:
        photo_good: bool
        quality_score: int, 0~100
        quality_problems: list[str]
        framing: dict with useful values for later camera movement
    """
    ...
    # 3. Vertical framing
    # ----------------------------------------------------
    # For portraits/group photos, the subject usually looks better
    # slightly above the exact center.
...
def draw_status_panel(img, photo_good, quality_score, quality_problems, adjustment):
    ...
    panel_x2, panel_y2 = 260, 88

    cv2.rectangle(img, (panel_x1, panel_y1), (panel_x2, panel_y2), (0, 0, 0), -1)
    ...
    lines = [
        (f"Q:{quality_score} {status_text}", status_color),
        (f"Pan:{adjustment['pan_dir']} {adjustment['pan_amount_deg']:.1f}", (255, 255, 255)),
        (f"Tilt:{adjustment['tilt_dir']} {adjustment['tilt_amount_deg']:.1f}", (255, 255, 255)),
        (f"Size:{adjustment['size_status']}", (255, 255, 255)),
        (f"Issue:{problem_text}", (255, 255, 255)),
    ]
    ...
    for text, color in lines:
        cv2.putText(img, text, (16, y), font, scale, color, thickness, cv2.LINE_AA)
        y += 15
...
try:
    while cap.isOpened():
        ...
        if len(indices) > 0:
            ...
            #     )
            # else:
            #     problem_text = quality_problems[0] if quality_problems else "Bad photo"
            ...
            print("Quality:", quality_score, "| Good:", photo_good, "| Problems:", quality_problems)
        
            adjustment = compute_camera_adjustment(framing, IMG_SIZE)

            print("Adjustment:", adjustment["summary"])
        ...
        ret, encoded_img = cv2.imencode('.jpg', display_img, [int(cv2.IMWRITE_JPEG_QUALITY), 80])
